...
with =nix-shell= (typically called "shell.nix").  For projects that provide a
Nix environment with the not-yet-released Nix flakes feature, please consider
using the =use_flake= function of the [[nixdirenv][nix-direnv]] project, which you can use
...
** Per-project configuration
   ...
   Direnv needs to know when to consider recalculating an environment's
   variables.  To do this, we need to register files to watch for changes.  This
   is what the "watch" switches above help specify.
...
* Known Limitations
   ...
   Also, soon to be released in a new version of Nix are [[nix-flakes][Nix flakes]].  If you
   don't know what flakes are, you may want to wait until they stabilize and are
   officially released.  If you're an early adopter of flakes, the [[nixdirenv][Nix-direnv]]
   project has support for Nix flakes with it's =use_flake= function.  Lorelei
   can be installed and used concurrently with other projects offering similar
   functionality (Lorri, Nix-direnv, and the rest).  You just make different
   calls in your project's =.envrc= files.  No core contributor of Lorelei is
   using this unreleased version of Nix supporting flakes, so we didn't want to
   provide something we had not tested ourselves.
